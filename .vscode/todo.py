...
class TodoApp:
    def __init__(self, root):
        ...
        self.task_listbox = tk.Listbox(root, width=50, height=15) #makes tasks to show
        ...
        self.reminder_thread = threading.Thread(target=self.remind, daemon=True) #starts reminder thread, remind method
            ...
            for t in self.tasks:
                if t["due_date"] and not t["completed"]:
                    if current_time >= t["due_date"]:
                        ...
                        t["completed"] = True
            self.refresh_task_list()
            threading.Event().wait(60)  #sleep for 60 sec
